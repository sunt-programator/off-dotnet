// <copyright file="SyntaxTriviaListTests.Enumerator.cs" company="Sunt Programator">
...
public partial class GreenNodeTests
{
    ...
    {
        // Arrange
        SyntaxTriviaList.Enumerator enumerator = default;
    ...
    [Fact(DisplayName = $"The {nameof(SyntaxTriviaList.Enumerator.Current)} property must throw a {nameof(InvalidOperationException)}")]
    ...
    {
        // Arrange
        var syntaxTriviaList = new SyntaxTriviaList(_syntaxToken, _triviaList, 56);
    ...
    public void Enumerator_MoveNextMethod_ShouldReturnTrue_IfUnderlyingNodeIsSingleNode()
    {
        // Arrange
        ...
        var syntaxTriviaList = new SyntaxTriviaList(_syntaxToken, triviaList, 56);
        var enumerator = syntaxTriviaList.GetEnumerator();
    ...
    [Fact(DisplayName = $"The {nameof(SyntaxTriviaList.Enumerator.Current)} property must return the current syntax trivia when the underlying node is a list")]
    public void Enumerator_CurrentProperty_MustReturnCurrentSyntaxTrivia_WhenUnderlyingNodeIsList()
    {
        // Arrange
        var syntaxTriviaList = new SyntaxTriviaList(_syntaxToken, _triviaList, 56, 10);
        var enumerator = syntaxTriviaList.GetEnumerator();
    ...
    [Fact(DisplayName =
        $"The {nameof(SyntaxTriviaList.Enumerator.Current)} property must return the current syntax trivia when the underlying node is a single node")]
    public void Enumerator_CurrentProperty_MustReturnCurrentSyntaxTrivia_WhenUnderlyingNodeIsSingleNode()
    {
        // Arrange
        ...
        var trivia2 = SyntaxFactory.Trivia(SyntaxKind.SingleLineCommentTrivia, "% comment");
        var triviaList = SyntaxFactory.Token(SyntaxKind.TrueKeyword, trivia1, trivia2);
        var syntaxTriviaList = new SyntaxTriviaList(_syntaxToken, triviaList, Position, Index);
        var enumerator = syntaxTriviaList.GetEnumerator();
    ...
    public void Enumerator_ResetMethod_ShouldResetEnumerator()
    {
        ...
        const int Position = 56;
        const int Index = 10;
        var syntaxTriviaList = new SyntaxTriviaList(_syntaxToken, _triviaList, Position, Index);
        var enumerator = syntaxTriviaList.GetEnumerator();
